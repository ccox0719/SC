...
(function(Core){
  ...
  Core.canApplyCardToShip = function(card, ship, ownerId){
    ...
    if(ownerId!==S.turn) return false; // own ships only during your turn
    if(card.suit==="C") return true;
    if(card.suit==="H") return true;
    if(card.suit==="D") return true; // set/refresh shield
...
App.flow = (function(){
  ...
  function applySpecialOn(targetId){
    ...
    if(card.suit==="JOKER"){
      ...
      if(!tgt || tgt.weapons.length===0){ S.history.pop(); return; }
